			...
			<form method="post" action="logic_reports_13thmonthpay_overall.php" id="dynamicForm">
				...
				<table class="table table-bordered pull-down">
					...
					<?php
						...
						// 			";
						// }	
					$employeeCount = count($_POST['empid']);

					for($count = 0; $count < $employeeCount; $count++)
					{
						Print "
							<tr id='".$_POST['empid'][$count]."'>
								<input type='hidden' value='".$_POST['empid'][$count]."' name='empid[]'>
								<input type='hidden' value='".numberExactFormat($_POST['payment'][$count], 2, '.',false)."' name='onetri[]'>
								<input type='hidden' value='".$_POST['startDate'][$count]."' name='startDate[]'>
								<input type='hidden' value='".$_POST['endDate'][$count]."' name='endDate[]'>
								<td align='left'>
									".$_POST['name'][$count]."
								</td>
								<td>
									".$_POST['position'][$count]."
								</td>
								<td>
									".$_POST['startDate'][$count]." - ".$_POST['endDate'][$count]."
								</td>
								<td class='onetriAmount'>
									".numberExactFormat($_POST['payment'][$count], 2, '.',true)."
								</td>
								<td>
									<input type='number' class='amount' name='amount[]' onchange='amountInput(\"".$_POST['empid'][$count]."\")'>
								</td>
								<td>
									<input type='checkbox' onchange='copyAmount(\"".$_POST['empid'][$count]."\")'>
								</td>
							</tr>";
					}
