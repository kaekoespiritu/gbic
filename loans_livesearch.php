...
if(!empty($result))
{
  if(mysql_num_rows($result) > 0)
  {
    ...
    //Pre set values of loans
        ...
        $employeeNewVale = 0;
        $empid = $row['empid'];
        $sss = "SELECT * FROM loans WHERE type = 'SSS' AND empid = '$empid' ORDER BY date DESC LIMIT 1";
        $pagibig = "SELECT * FROM loans WHERE type = 'PagIBIG' AND empid = '$empid' ORDER BY date DESC LIMIT 1";
        $oldVale = "SELECT * FROM loans WHERE type = 'oldVale' AND empid = '$empid' ORDER BY date DESC LIMIT 1";
        $newVale = "SELECT * FROM loans WHERE type = 'newVale' AND empid = '$empid' ORDER BY date DESC LIMIT 1";
